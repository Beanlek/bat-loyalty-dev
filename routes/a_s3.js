...
router.post('/uploadReceipt', apiAuth.checkToken, upload.single('file'), async (req, res) => {
    ...
    try{ 
        ...
        try { 
            ...
            return res.status(500).send({ status: "Failed to upload the image" });
        }

    }catch(e){ 
        console.error("Error occurred:", e.message);
        console.error("Stack trace:", e.stack);  
        return res.status(500).send({ status: "No outlet ID is assigned to the user" });
    } 
    biz.s3.uploadFile(req, res); 
}); 
