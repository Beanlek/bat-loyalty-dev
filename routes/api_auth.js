...
router.post('/login', async (req, res) => {
    ...
    let user = await db.users.findOne({
        where: db.Sequelize.where(db.Sequelize.fn('lower', db.Sequelize.col('mobile')), sq.fn('lower', mobile)),raw: true
    });
    ...
    // let expires_in = await db.parameters.findOne({
    //     where:{
    //     param_group:{[Op.eq]: 'backend'},
    //     param_id: {[Op.eq]: 'token_expires_in'}
    //     },raw: true
    // })
    ...
    return res.json({token, token_expiry_time, allotment_date});
    }
);
