...
S3.deleteFile = async function (req, res){ 
  ...
  if(!receiptImageId) {return res.status(400).send({status: 'Receipt image ID is required'})}
  ...
  try { 
    ...
    try {
      ...
      return res.status(500).send({ status: 'Image deleted from DB, but failed to delete from AWS' });
    }

  } catch (dbError) {
    if (transaction) await transaction.rollback();
    console.error("Error deleting image from DB:", dbError);
    return res.status(500).send({ status: 'Error deleting image from DB' });
  }
